...
`Tagged<Tag, RawValue>` in swift-tagged-primitives was inspired by Point-Free's [swift-tagged](https://github.com/pointfreeco/swift-tagged). Both libraries solve the same fundamental problem: wrapping a raw value with a phantom type parameter for compile-time type safety. However, the implementations have diverged significantly in design philosophy, capability, and rigor.
...
|--------|-------------|--------------------------|
| Declaration | `struct Tagged<Tag, RawValue>` | `struct Tagged<Tag: ~Copyable, RawValue: ~Copyable>: ~Copyable` |
| Tag constraint | `Tag: Copyable` (implicit) | `Tag: ~Copyable` |
| RawValue constraint | `RawValue: Copyable` (implicit) | `RawValue: ~Copyable` |
| Copyable conformance | Always (unconditional) | Conditional: `where RawValue: Copyable` |
| ~Copyable tag support | No | Yes — `Index<Element>` where `Element: ~Copyable` |
| ~Copyable raw value support | No | Yes — `Tagged<Tag, Resource>` where `Resource: ~Copyable` |
...
Experiment `tagged-noncopyable-rawvalue` (2026-01-24, CONFIRMED) verified that `Tagged` with `~Copyable` raw values works correctly with `Equation.Protocol`, `Comparison.Protocol`, and `Hash.Protocol` from the respective primitives packages.
...
| Aspect | swift-tagged | swift-tagged-primitives |
|--------|-------------|--------------------------|
| Storage | `public var rawValue: RawValue` | `@usableFromInline internal var _storage: RawValue` |
| Read access | Direct property access | `_read` coroutine via computed `rawValue` |
| Write access | Direct property access | `_modify` coroutine via computed `rawValue` |
...
| Aspect | swift-tagged | swift-tagged-primitives |
|--------|-------------|--------------------------|
| Primary init | `init(rawValue:)` | `init(__unchecked: Void, _ rawValue: consuming RawValue)` |
| Convenience init | `init(_ rawValue)` (positional) | None in production |
| `consuming` parameter | No | Yes |
| `RawRepresentable` | Yes (conforms) | No |

The `__unchecked:` label is deliberate — it signals that the initializer bypasses domain-specific validation. Domain types (e.g., `Index<Element>`) provide their own validated initializers and call `__unchecked:` internally. swift-tagged's `init(rawValue:)` / `init(_:)` encourages direct construction at all call sites.

The `consuming` ownership annotation enables move-only raw values and avoids unnecessary copies.
...
swift-tagged applies `@dynamicMemberLookup` to forward `KeyPath` subscripts to `rawValue`. This enables `tagged.someProperty` syntax without unwrapping. We deliberately omit this — it conflicts with the principle that phantom-typed values should be explicitly unwrapped before accessing raw value members. Implicit forwarding obscures the type boundary.
...
**Reason**: `Strideable` enables `Tagged` values to be used in `for x in a...b` ranges and implies a linear ordering with uniform stride. This is inappropriate for phantom-typed values — `Index<Graph>` should not be strideable because the stride semantics depend on the domain, not the raw value. Domain-specific stride operations belong in domain-specific extensions (e.g., `Index Primitives`), not as a blanket forwarding.
...
**Reason**: We use `@usableFromInline internal var _storage` with computed `rawValue` via `_read`/`_modify` coroutines. This separates the storage representation from the access interface, allowing future storage changes without ABI breakage.
...
**Reason**: A `Tagged<Tag, [Int]>` that itself conforms to `Collection` conflates the wrapper with its contents. If you want to iterate the raw value, unwrap it: `tagged.rawValue.forEach { ... }`. Making the wrapper itself iterable obscures the type boundary.
...
**Reason**: `Tagged` *is* an identity mechanism. Making it `Identifiable` by forwarding to `rawValue.id` creates a semantic confusion — the tag is the identity discriminator, not the raw value's `id` property. Domain types should conform to `Identifiable` directly.

#### 3.9 `LosslessStringConvertible`

swift-tagged conditionally conforms to `LosslessStringConvertible`.

**Reason**: `LosslessStringConvertible` implies a round-trip guarantee (`init?(_:) → description → init?(_:)`). This guarantee is about the raw value, not the tagged value. The tag information is lost in the string representation, so the conversion is inherently lossy from the Tagged perspective.
...
**Note**: We also conditionally conform to `BitwiseCopyable` (`where Tag: ~Copyable, RawValue: BitwiseCopyable`). This is not a removal — both libraries provide this conformance. The conditional gating on `RawValue: BitwiseCopyable` ensures noncopyable raw values are not affected.
...
These coroutines provide zero-copy read and write access to the stored value. For large `RawValue` types, this avoids the copy that a simple `get`/`set` pair would introduce. swift-tagged's `public var rawValue` has equivalent performance only because it's a stored property — if it were ever changed to computed, performance would degrade without coroutines.
...
Package-internal mutation access for performance-critical paths within the package where wrapping and unwrapping via `rawValue` would be suboptimal. The `package` access level restricts this to the defining package, preventing misuse by downstream consumers.
...
This split enables `consuming` semantics for `~Copyable` raw values in the static form, while the instance form provides ergonomic call-site syntax. swift-tagged has only the instance form.
...
Every extension specifies `Tag: ~Copyable` and `RawValue: ~Copyable` (or `RawValue: Copyable` for conformances that require it). This ensures the phantom type system does not exclude noncopyable types. swift-tagged has no `~Copyable` awareness.
...
`Tagged` is `Copyable` when its raw value is, and `~Copyable` when it isn't. This is the correct conditional behavior for a wrapper type. swift-tagged is unconditionally `Copyable`.
...
The `Tag: ~Copyable` ensures `Sendable` conformance applies to all tags, and `RawValue: ~Copyable & Sendable` enables sendable noncopyable raw values. swift-tagged requires `RawValue: Sendable` (implicitly `Copyable`).

#### 4.9 `#if !hasFeature(Embedded)` Guard on Codable

```swift
#if !hasFeature(Embedded)
    extension Tagged: Codable where Tag: ~Copyable, RawValue: Codable {}
#endif
```
...
| `RawRepresentable` | **Absent everywhere** | HARD: structural Swift-level blocker — protocol not `~Escapable`-aware; Tagged's structural `~Escapable` propagates through synthesized rawValue getter witness | [doc](./principled-absence-rawrepresentable.md) |
...
| Decodable fallback (`try RawValue(from: decoder)`) | **Absent everywhere** | HARD: anti-pattern — masks decoding errors; our simple conditional `Codable` is correct | [doc](./principled-absence-niche-protocols.md) |
...
Every Tagged operation — init, rawValue access, retag, Comparable delegation, map with identity — compiles to identical instructions as the raw equivalent.
...
| Aspect | swift-tagged | swift-tagged-primitives |
|--------|-------------|--------------------------|
| Literal conformances | Production (blanket) | Test support module only |
...
| Companion modules | None | `Tagged Primitives Test Support` (quarantined literals) |

The ecosystem integration philosophies differ fundamentally:

**swift-tagged**: Maximize convenience. Provide every conformance that could be useful. Let Tagged work as a drop-in substitute for the raw value in most contexts. Trade type safety for ergonomics where the two conflict (arithmetic, literals, dynamic member lookup).

**swift-tagged-primitives**: Maximize type safety. Provide only conformances that preserve the phantom type's discrimination guarantee. Require explicit domain-specific extensions for operations that depend on the tag. Quarantine convenience features (literals) that undermine safety. The result is more boilerplate for domain types but stronger compile-time guarantees.

The `Tagged Primitives Test Support` module is the key architectural difference. By isolating literal conformances to a test-only dependency, we get ergonomic test code (`let x: Tagged<Tag, Int> = 42`) without contaminating production code with the overload resolution footgun documented in `tagged-literal-conformances.md`.
...
| `~Copyable` | Not supported | Full support (Tag + RawValue) |
...
| Conformance count | ~25 conditional conformances | 8 conditional conformances |
| Source files | 2 (+ 2 nanolibraries) | 2 (+ 1 test support) |
...
2. **~Copyable support** — `consuming` parameters, conditional `Copyable`, Tag/RawValue `~Copyable`
...
- `swift-tagged-primitives/Tests/Support/Tagged Primitives Test Support.swift` — quarantined literal conformances
